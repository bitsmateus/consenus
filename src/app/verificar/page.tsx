...
export default async function PaginaDeVerificacao({
  ...
  // { existe, tipo, dataEmissao, numeroDoAto } — nunca o arquivo.

  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col justify-center px-6 py-12">
      <div className="mb-8 border-l-4 border-dourado-600 pl-4">
        <h1 className="font-serif text-2xl text-marinho-900">Consensus One</h1>
        <p className="text-sm text-ardosia-500">Câmara Privada de Conciliação</p>
      </div>

      <h2 className="mb-2 text-xl font-semibold text-marinho-700">
        Verificação de autenticidade
      </h2>
      <p className="mb-6 text-sm text-ardosia-500">
      ...
      <form method="get" className="flex flex-col gap-3 sm:flex-row">
        <input
          ...
          placeholder="CO-2026-0001-A3F7K2"
          className="tabular flex-1 rounded-md border border-ardosia-100 bg-superficie px-4 py-3 uppercase outline-none focus:border-marinho-500"
          aria-label="Código do documento"
        />
        <button
          type="submit"
          className="rounded-md bg-marinho-700 px-6 py-3 font-medium text-white transition-colors hover:bg-marinho-500"
        >
          Verificar
        </button>
      </form>
      ...
      {formatoValido === false && (
        <p className="mt-4 text-sm text-erro">
